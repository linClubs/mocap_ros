...
static bool BytestoCalculationData(unsigned char *bytes, struct_ReceivedMotionData &calculationData)
{
    ...
    if ((unsigned short)(bytes[2] << 8 | bytes[3]) - 3 < (NODES_BODY * 8))
    {
        return false;
    }
    ...
    if (bytes[offset] == 1)
    {
        calculationData.isUpdate = true;
    }
    else
    {
        calculationData.isUpdate = false;
    }
    ...
    if (bytes[offset] == 1)
    {
        calculationData.isUseFaceBlendShapesARKit = true;
    }
    else
    {
        calculationData.isUseFaceBlendShapesARKit = false;
    }
    ...
    if (bytes[offset] == 1)
    {
        calculationData.isUseFaceBlendShapesAudio = true;
    }
    else
    {
        calculationData.isUseFaceBlendShapesAudio = false;
    }
